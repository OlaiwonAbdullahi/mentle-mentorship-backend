...
const createEnrollment = async (req, res) => {
    try {
        ...
        const enrollmentData = {
            ...
            successMeasurement,
            furtherQuestions,
            willingToAttendNext: willingToAttendNext === "true" || willingToAttendNext === true,
            paymentMethod,
            status: "pending",
        };
        ...
        if (location === "Europe") {
            if (!feeCommitment || feeCommitment === "false") {
                return res.status(400).json({
                    success: false,
                    message: "Fee commitment confirmation is required for European applicants",
                });
            }
